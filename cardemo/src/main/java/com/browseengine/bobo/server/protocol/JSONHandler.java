...
public class JSONHandler extends ProtocolHandler {
  ...
  public Object deserializeRequest(Class<?> reqClass, HttpServletRequest req) throws IOException {
    ...
    if (null == reqString) {
      throw new IOException("no 'req' parameter specified on requet for deserialization of class "
          + reqClass.toString());
    }
    try {
      ...
      throw new IOException("deserialize request with class " + reqClass.toString() + ": "
          + e.toString());
    }
  }
