...
public class DispatchUtil {
  ...
  private static class DispatchSolrParams extends SolrParams {
    ...
    private final SolrParams _params;
  ...
  public static BrowseResult broadcast(ExecutorService threadPool, SolrParams boboSolrParams,
      BrowseRequest req, String[] baseURL, int maxRetry) {
    long start = System.currentTimeMillis();
    Future<BrowseResult>[] futureList = new Future[baseURL.length];
    ...
    Map<String, FacetAccessible> mergedFacetMap = ListMerger.mergeSimpleFacetContainers(facetList,
      req);
    ...
    List<BrowseHit> mergedList = req.getCount() > 0 ? ListMerger.mergeLists(req.getOffset(),
      req.getCount(), iteratorList.toArray(new Iterator[iteratorList.size()]), comparator)
  ...
  private static BrowseResult doShardCall(SolrServer solrSvr, SolrParams params, String baseURL,
      int maxRetry) throws SolrException, SolrServerException, IOException {
    return parseResponse(solrSvr.query(params));
  }

  private static Callable<BrowseResult> newCallable(final SolrParams req, final String baseURL,
      final int maxRetry) {
