...
public class MultiBoboBrowser extends MultiSearcher implements Browsable,Closeable
{
 ...
 // index empty
    ...
    SortCollector collector = getSortCollector(req.getSort(),req.getQuery(), offset, count, req.isFetchStoredFields(), req.isFetchStoredValue(),req.getTermVectorsToFetch(),false, req.getGroupBy(), req.getMaxPerGroup(), req.getCollectDocIdCache());
  ...
  public SortCollector getSortCollector(SortField[] sort, Query q,int offset, int count, boolean fetchStoredFields, boolean fetchStoredValue,Set<String> termVectorsToFetch,
		boolean forceScoring, String groupBy, int maxPerGroup, boolean collectDocIdCache) {
	if (_subBrowsers.length==1){
		return _subBrowsers[0].getSortCollector(sort, q, offset, count, fetchStoredFields, fetchStoredValue, termVectorsToFetch,forceScoring, groupBy, maxPerGroup, collectDocIdCache);
	}
    return SortCollector.buildSortCollector(this, q, sort, offset, count, forceScoring, fetchStoredFields, fetchStoredValue, termVectorsToFetch,groupBy, maxPerGroup, collectDocIdCache);
  }
